...
class TV_GaussianProcess(object):
    ...
    # x: nxp, y: nx1
    def fit(self, x, y, Dt, lr=0.01, iters=100, restarts=0):
        b = x.size(0)  # n
        n = x.size(1)
        x1 = x.unsqueeze(1).expand(b, b, n)
        x2 = x.unsqueeze(0).expand(b, b, n)
        ...
        self.x = x
        self.y = y
        k = self.kernel(x1, x2).view(b, b) * Dt + self.alpha * torch.eye(b)
        self.kinv = torch.inverse(k)

    def predict(self, x, dt, return_std=False):
        b_prime = x.size(0)
        b = self.x.size(0)
        n = self.x.size(1)
        x1 = x.unsqueeze(1).expand(b_prime, b, n)
        x2 = self.x.unsqueeze(0).expand(b_prime, b, n)
        k = self.kernel(x1, x2).view(b_prime, b) * dt.repeat(1, b_prime).T
        mu = k @ self.kinv @ self.y
        x = x.unsqueeze(1).expand(b_prime, 1, n)
        sigma = self.kernel(x, x) - (k @ self.kinv @ k.t()).diag().view(mu.size())
        if return_std:
