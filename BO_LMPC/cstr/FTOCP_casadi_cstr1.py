...
from casadi import *
import sys
sys.path.append('../mrpi')
from BO_LMPC.mrpi.polyhedron import polyhedron, plot_polygon_list
from BO_LMPC.mrpi.mRPI_set import compute_mRPI
...
class FTOCP(object):
    ...
    def compute_mrpi(self):
        self.mrpi, self.F_list = compute_mRPI(1e-10, self.W, self.A, self.B, self.K, 100)

        # 处理一下mrpi
        self.mrpi.minVrep()
        # 这是在干嘛，好像小数点位数过多会报错？
        try:
            self.mrpi.vertices = np.round(self.mrpi.vertices, 3)
            self.mrpi.compute_Hrep()
        except RuntimeError:
            self.mrpi.vertices = np.round(self.mrpi.vertices)
            self.mrpi.compute_Hrep()

        K_mrpi = self.mrpi.affineMap(self.K)
    ...
    def solve(self, x0, time=0, verbose=False, SS=None, Qfun=None, CVX=None):
        ...
        for i in range(self.N+1):
            if time + i < self.s + 1:
                ...
                self.len_conx += self.mrpi.A.shape[0]

        for i in range(self.N):
            if time + i < self.s + 1:
                self.len_conu += self.constr_u[time+i].A.shape[0]
            else:
                self.len_conu += self.mrpi_u.A.shape[0]
        ...
        for i in range(self.N+1):
            if time + i < self.s + 1:
                ...
                constraints = vertcat(constraints,
                                      mtimes(self.mrpi.A, x[self.n * i:self.n * (i + 1)]) - self.mrpi.b)
        for i in range(self.N):
            if time + i < self.s + 1:
                constraints = vertcat(constraints, mtimes(self.constr_u[time+i].A,
                                                      u[self.d * i:self.d * (i + 1)])-self.constr_u[time+i].b)
            else:
                constraints = vertcat(constraints, mtimes(self.mrpi_u.A,
                                                          u[self.d * i:self.d * (i + 1)]) - self.mrpi_u.b)
