...
ax[1].set_title('transfer learning bo')
...
for i in range(5):
    ...
    ax[1].arrow(i+1, ucl_tvbo[i]+tb[i], 0, l1, color='r')
    ax[1].arrow(i+1, ucl_lmpc[i]+lm[i], 0, -l1, color='r')

    ax[1].arrow(i+1, ucl_tvbo[i]-tb[i], 0, -l2, color='r')
    ax[1].arrow(i+1, ucl_lmpc[i]-lm[1], 0, l2, color='r')
