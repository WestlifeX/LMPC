...
class LMPC(object):
    ...
    def addTrajectory(self, x, u):
        ...
        self.xGuess = np.concatenate((x[:, 0:(self.ftocp.N + 1)].T.flatten(), u[:, 0:(self.ftocp.N)].T.flatten()),
                                     axis=0)
    ...
    def solve(self, xt, verbose=5):
        ...
        if ((indList[bestItLocSS][bestTime] + 1) <= (self.SS[bestIt].shape[1] - 1)):
            ...
            self.xGuess[(n * (N + 1) + d * (N - 1)):(n * (N + 1) + d * N)] = self.uSS[bestIt][:,
                                                                             indList[bestItLocSS][bestTime]]
        else:
