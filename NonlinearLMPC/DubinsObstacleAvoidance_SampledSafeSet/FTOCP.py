...
class FTOCP(object):
    ...
    def solve(self, x0, xf):
        ...
        self.lbx = x0 + [-1000] * (self.n * (self.N)) + [-np.pi / 2.0, -1.0] * self.N + [-1000] * self.n + [1] * (
                    self.N - 1)
        self.ubx = x0 + [1000] * (self.n * (self.N)) + [np.pi / 2.0, 1.0] * self.N + [1000] * self.n + [100000] * (
                    self.N - 1)
    ...
    def buildNonlinearProgram(self, N, xf):
        ...
        for i in range(1, N):
            constraint = vertcat(constraint,
                                 ((X[n * i + 0] - 27.0) ** 2 / 64.0) + ((X[n * i + 1] + 1.0) ** 2 / 36.0) - slackObs[
                                     i - 1])
        ...
        opts = {"verbose": False, "ipopt.print_level": 0, "print_time": 0, "ipopt.mu_strategy": "adaptive",
                ...
                "ipopt.barrier_tol_factor": 1}  # , "ipopt.acceptable_constr_viol_tol":0.001}#,"ipopt.acceptable_tol":1e-4}#, "expand":True}
